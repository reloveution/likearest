...
class Sort {
    // restApiArk получаем список отзывов о продавце и сортируем по дате
     sortFeedbacksForSellerByDate(sellerID, sortOrder = 'des') {
        const sellerApi = new SellerApi()
        const feedbackApi = new FeedbackApi

        let sellerModel
        let feedbackBySellerModel

        sellerApi.get()
        .then(result => {
            sellerModel = result
            return feedbackApi.getFeedbacksBySeller(sellerID)
        })
        .then(response => {
            // console.log(response)
            // feedbackBySellerModel = FeedbackUtils.sortByDate(response, sortOrder)
            console.log(FeedbackUtils.sortByDate(response, sortOrder))
        })
        // .then(() => {

        //     let tempFeedback
        //     for (let ii = 0; ii < (feedbackBySellerModel.length - 1); ii++) {
        //         for (let i = 0; i < (feedbackBySellerModel.length - 1); i++) {
        //             if (Date.parse(feedbackBySellerModel[i].feedbackInfo.date) < Date.parse(feedbackBySellerModel[i+1].feedbackInfo.date)) {
        //                 tempFeedback = feedbackBySellerModel[i]
        //                 feedbackBySellerModel[i] = feedbackBySellerModel[i+1]
        //                 feedbackBySellerModel[i+1] = tempFeedback
        //             }
        //         }
        //     }
        //     console.log('-----Sort on date------')
        //     console.log(feedbackBySellerModel)
        // })            
    }
